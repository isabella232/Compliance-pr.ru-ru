...
В этой модели нет единого источника данных каталогов. Конкретные системы владеют отдельными частями данных, но ни одна система не содержит все данные. Службы Microsoft 365 сотрудничают с Azure AD в этой модели данных. Azure AD — это "система правды" для общих данных, которая обычно является небольшими и статическими данными, используемыми каждой службой. Федерарная модель, используемая в Microsoft 365 и Azure AD, предоставляет общее представление данных.

Microsoft 365 использует физическое хранилище и облачное хранилище Azure. Exchange Online (в том числе Exchange Online Protection) и Skype для бизнеса используют собственное хранилище для данных клиентов. SharePoint Online использует SQL Server и Azure Storage, поэтому требуется дополнительная изоляция данных клиентов на уровне хранилища.

## <a name="exchange-online"></a>Exchange Online

Exchange Online хранит данные клиентов в почтовых ящиках. Почтовые ящики находятся в базах данных Extensible Storage Engine (ESE), называемых базами данных почтовых ящиков. К ним относятся почтовые ящики пользователей, связанные почтовые ящики, общие почтовые ящики и почтовые ящики общедоступных папок. Почтовые ящики пользователей включают сохраненный контент Skype для бизнеса, например истории разговоров.
...
SharePoint Online использует SQL Server и Azure Storage для хранения метаданных контента. Ключ раздела для магазина контента *— SiteId* в SQL. При запуске SQL SharePoint Online используется *проверка SiteId* в рамках проверки *SubscriptionId* на уровне клиента.

SharePoint Online хранит зашифрованное содержимое файлов в blobs Microsoft Azure. Каждая ферма SharePoint Online имеет свою учетную запись Microsoft Azure, и все blobs, сохраненные в Azure, шифруются отдельно с ключом, хранимым в SQL хранилище контента. Ключ шифрования, защищенный в коде слоем авторизации и не подвергался непосредственному контакту с конечным пользователем. SharePoint Online имеет мониторинг в режиме реального времени, чтобы определить, когда запрос HTTP читает или записывает данные для более чем одного клиента. Идентификатор *запроса SubscriptionId* отслеживается в *subscriptionId* доступного ресурса. Запросы на доступ к ресурсам более чем одного клиента никогда не должны происходить конечными пользователями. Единственным исключением являются запросы на обслуживание в среде с несколькими клиентами. Например, обходной запрос извлекает изменения контента для всей базы данных одновременно. Это обычно включает запрос сайтов более чем одного клиента в одном запросе службы, который делается по соображениям эффективности.

## <a name="teams"></a>Teams

Данные Teams хранятся по-разному в зависимости от типа контента. 

Ознакомьтесь с [сеансом Ignite breakout в архитектуре Microsoft Teams](https://channel9.msdn.com/Events/Ignite/Microsoft-Ignite-Orlando-2017/BRK3071) для углубленного обсуждения.

### <a name="core-teams-customer-data"></a>Данные клиентов Core Teams
...
- Командные чаты, беседы с группами и каналами, изображения, сообщения голосовой почты и контакты.
...
Каждая команда в Teams имеет группу Microsoft 365 и ее сайт SharePoint и почтовый ящик Exchange. Частные чаты (включая групповые чаты), сообщения, отправленные в рамках беседы на канале, а также структура групп и каналов хранятся в службе чата, запущенной в Azure. Данные также хранятся в скрытой папке в почтовых ящиках пользователей и групп, чтобы включить функции защиты информации.

#### <a name="voicemail-and-contacts"></a>Голосовая почта и контакты

Голосовая почта хранится в Exchange. Контакты хранятся в хранилище облачных данных на основе Exchange. Exchange и облачный магазин Exchange уже предоставляют резидентство данных в каждом из геоцентров обработки данных по всему миру. Для всех групп голосовая почта и контакты хранятся в австралии, Канаде, Франции, Германии, Индии, Японии, Объединенных Арабских Эмиратах, Великобритании, Южной Африке, Южной Корее, Швейцарии (включая Лихтенштейн) и США. Для всех других стран файлы хранятся в США, Европе или Asia-Pacific на основе сродства клиента.
...
Файлы (в том числе OneNote и Wiki), которые кто-то делит в канале, хранятся на сайте SharePoint группы. Файлы, общие в частном чате или чате во время собрания или звонка, загружаются и хранятся в учетной записи OneDrive для работы или учебного заведения пользователя, который разделяет файл. Exchange, SharePoint и OneDrive уже предоставляют резидентство данных в каждом из геоцентров обработки данных по всему миру. Таким образом, для существующих клиентов все файлы, записные книжки OneNote, контент вики-команд и почтовые ящики, которые являются частью работы Teams, уже хранятся в расположении на основе сродства клиента. Файлы хранятся в Австралии, Канаде, Франции, Германии, Индии, Японии, Объединенных Арабских Эмиратах, Великобритании, Южной Африке, Южной Корее и Швейцарии (включая Лихтенштейн). Для всех других стран файлы хранятся в США, Европе или Азиатско-Тихоокеанском регионе на основе сродства клиента.
